...
def create_product_content_tasks_with_emotion(product_name, output_directory, Agents):
    ...
    source_validation_task = Task(
        description=f"Identify and validate reliable websites and sources for researching detailed information about {product_name}. Ensure sources provide insights into consumer emotions and preferences related to the product.",
        expected_output="A list of validated and reliable sources for product research, focusing on emotional or storytelling elements.",
        agent=Agents[0]
    )

    # Tarefa 2: Pesquisa Detalhada sobre o Produto
    product_research_task = Task(
        description=f"Collect detailed information about {product_name}, including specifications, features, usage scenarios, and any emotional or storytelling angles from validated sources.",
    ...
    competitor_comparison_task = Task(
        description=f"Compare {product_name} with similar products in the market. Identify unique strengths, weaknesses, and features, while highlighting emotional or narrative aspects that make {product_name} stand out.",
    ...
    review_analysis_task = Task(
        description=f"Analyze consumer reviews and feedback for {product_name}. Identify recurring themes, emotional reactions, and any user stories that could be leveraged for creating engaging narratives about the product.",
    ...
    content_idea_generation_task = Task(
        description=f"Generate creative content ideas for {product_name}, such as blog posts, social media captions, or promotional materials. Incorporate emotional storytelling elements, like excitement, nostalgia, or trust, to engage the audience effectively.",
    ...
    content_creation_task = Task(
        description=f"Write high-quality content about {product_name}, integrating storytelling techniques and evoking emotions such as trust, excitement, or connection based on the research and consumer feedback.",
    ...
    # Tarefa 7: Análise de Emoção e Narrativa
    emotion_narrative_task = Task(
        description=f"Analyze the product research, consumer feedback, and content ideas to suggest the most effective emotions or narrative techniques to use in the final content.",
        ...
        output_file=os.path.join(output_directory, "6-emotion-narrative.md")
    )
